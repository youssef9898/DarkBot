...
client = Bot(description="DarkBot Bot is best", command_prefix="R", pm_help = True)
client.remove_command('help')


async def status_task():
    while True:
        await client.change_presence(game=discord.Game(name='for Rhelp'))
...
def is_owner(ctx):
    return ctx.message.author.id == "286867950466629642, 401751095153786890"

def is_dark(ctx):
    return ctx.message.author.id == "286867950466629642"
...
async def warn(ctx, userName: discord.User, *, message:str): 
    ...
    await client.say(":warning: __**{0} Has Been WarneR**__ :warning: ** Reason:{1}** ".format(userName,message))
    pass
...
async def help(ctx):
    ...
    embed.add_field(name = '``Our Help Server Link`` ',value ='https://discord.gg/xCAhhex',inline = False)
    embed.add_field(name = 'Rmodhelp ',value ='Explaines all the commands which are only usable by Those who has moderation permissions. Like- Manage Nicknames, Manage Messages, Kick/Ban Members,etc.',inline = False)
    embed.add_field(name = 'Rgeneralhelp ',value ='Explaines all the commands which are usable by everyone.',inline = False)
    await client.send_message(author,embed=embed)
...
async def modhelp(ctx):
    ...
    embed.set_image(url = 'https://image.ibb.co/caM2BK/help.gif')
    embed.add_field(name = 'Rsay(Admin permission required) ',value ='Use it like ``Rsay <text>``',inline = False)
    embed.add_field(name = 'Rembed(Admin permission required) ',value ='Use it like ``Rembed <text>``',inline = False)
    embed.add_field(name = 'Rmembercount(Kick members Permission Required) ',value ='Use it like ``Rmembercount`` to get membercount',inline = False)
    embed.add_field(name = 'Rremovemod(Admin Permission Required)',value ='Use it like ``Rremovemod @user`` to remove him from mod. Note-You need Moderator role in your server below darkbot to use it.',inline = False)
    embed.add_field(name = 'Rmakemod(Admin Permission Required)',value ='Use it like ``Rmakemod @user`` to make him mod. Note-You need Moderator role in your server below darkbot to use it.',inline = False)
    embed.add_field(name = 'Rsetup(Admin Permission Required)',value ='Use it to add channels, voice channels and roles if your server is not developed currently and you have just 1-2 channels. Note- Use it only 1 time. If you will use same command again then it will do same thing again .i.e It will add true copy of previous channels + true copy of roles that made in previous command use. So be careful.',inline = False)
    embed.add_field(name = 'Rfriend(Admin Permission Required) ',value ='Use it like ``Rfriend @user`` to give anyone Friend of Owner role',inline = False)
    embed.add_field(name = 'Rrole(Manage Roles Permission Required)',value ='Use it like ``Rrole @user <rolename>``.',inline = False)
    embed.add_field(name = 'Rsetnick(Manage nickname permission required)',value ='Use it like ``Rsetnick @user <New nickname>`` to change the nickname of tagged user.',inline = False)
    embed.add_field(name = 'Renglish(Kick members Permission Required)',value ='Use it like ``Renglish @user`` when someone speaks languages other than English.',inline = False)
    embed.add_field(name = 'Rserverinfo(Kick members Permission Required) ',value ='Use it like ``Rserverinfo`` to get server info',inline = False)
    embed.add_field(name = 'Ruserinfo(Kick members Permission Required) ',value ='Use it like ``Ruserinfo @user`` to get some basic info of tagged user',inline = False)
    embed.add_field(name = 'Rkick(Kick members Permission Required)',value ='Use it like ``Rkick @user`` to kick any user',inline = False)
    embed.add_field(name = 'Rroles(Kick members Permission Required) ',value ='Use it to check roles present in server',inline = False)
    embed.add_field(name = 'Rclear(Manage Messages Permission Required)',value ='Use it like ``Rclear <number>`` to clear any message',inline = False)
    embed.add_field(name = 'Rmute(Mute members Permission Required)',value ='Use it like ``Rmute @user <time>`` to mute any user',inline = False)
    embed.add_field(name = 'Runmute(Mute members Permission Required) ',value ='Use it like ``Runmute @user`` to unmute anyone',inline = False)
    embed.add_field(name = 'Rban(Ban members Permission Required) ',value ='Use it like ``Rban @user`` to ban any user',inline = False)
    embed.add_field(name = 'Rrules(Kick members Permission Required)',value ='Use it like ``Rrules @user <violation type>`` to warn user',inline = False)
    embed.add_field(name = 'Rwarn(Kick members Permission Required)',value ='Use it like ``Rwarn @user <violation type>`` to warn any user',inline = False)    
    embed.add_field(name = 'Rnorole(Kick members Permission Required) ',value ='Use it like ``Rnorole @user`` to warn anyone if he/she asks for promotion',inline = False)
    embed.add_field(name = 'Rgetuser(Kick members Permission Required) ',value ='Use it like ``Rgetuser @rolename`` to get list of all users having a particular role',inline = False)
    await client.send_message(author,embed=embed)
    await client.say('?? Check DMs For Information')
...
async def generalhelp(ctx):
    ...
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    embed = discord.Embed(color = discord.Color((r << 16) + (g << 8) + b))
    embed.add_field(name = 'Rpoll ',value ='Use it like ``Rpoll "Question" "Option1" "Option2" ..... "Option9"``.',inline = False)
    embed.add_field(name = 'Rguess ',value ='To play guess game use ``Rguess <number> and number should be between 1-10``',inline = False)
    embed.add_field(name = 'Rgithub ',value ='Use it like- ``Rgithub uksoftworld/DarkBot``',inline = False)
    embed.add_field(name = 'Rbottutorial ',value ='Use it like ``Rbottutorial <tutorial name by darklegend>``',inline = False)
    embed.add_field(name = 'Rdyno ',value ='Use it like ``RRdyno <dyno command name>``',inline = False)
    embed.add_field(name = 'Rdonate ',value ='Use it to donate us and get a special post on Official DarkBot server.',inline = False)
    embed.add_field(name = 'Rownerinfo ',value ='To get basic information about owner.',inline = False)
    embed.add_field(name = 'Rsourcecode ',value ='Use it to see darkbot sourcecode.',inline = False)
    embed.add_field(name = 'Rupvote ',value ='Use it to Upvote our bot and help us to grow',inline = False)
    embed.add_field(name = 'Rauthlink ',value ='Use it to get authorizing link to authorize this bot to your server.',inline = False)
    embed.add_field(name = 'Rhappybirthday @user ',value ='To wish someone happy birthday',inline = False)
    embed.add_field(name = 'Rtechnews ',value ='Use it to get tech news',inline = False)
    embed.add_field(name = 'Rgooglefy ',value ='Use it like ``Rgooglefy <string>``.',inline = False)
    embed.add_field(name = 'Rspacenews ',value ='Use it to get space news',inline = False)
    embed.add_field(name = 'Rphynews ',value ='Use it to get physycs',inline = False)
    embed.add_field(name = 'Rverify ',value ='Use it to get verified role. Note- It needs proper setup.',inline = False)
    embed.add_field(name = 'Rflipcoin ',value ='Flipps coin',inline = False)
    embed.add_field(name = 'Rrolldice ',value ='Rolls dice',inline = False)
    embed.add_field(name = 'Ravatar @user ',value ='Shows avatar',inline = False) 	
    await client.send_message(author,embed=embed)
    await client.say('?? Check DMs For Information')
